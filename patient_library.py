from My_array import PriorityQueueArray
...
class PatientManager:
    def __init__(self):
        """
        Initialize the PatientManager with a PriorityQueueArray for storing patients
        and a condition variable to notify the ICU.
        """
        self.patients = PriorityQueueArray()  # Use the PriorityQueueArray implementation
    ...
    def pop(self):
        ...
        with self.lock:
            if not self.patients:
                ...
                return None
            patient = self.patients.extract_max()
    ...
    def list_patients(self):
        ...
        with self.lock:
            ...
            for patient in self.patients.list_all():
                print(f"{patient.name} (Priority: {patient.esi_priority}, Arrival Time: {patient.arrival_time.strftime('%Y-%m-%d %H:%M:%S')})")
        ...
        def icu_worker():
            while True:
                ...
                while True:
                    ...
                    if patient:
                        operation_time = random.randint(30, 120)  # Random operation time
                        print(f"Operating on {patient.name} for {operation_time} seconds.")
                        time.sleep(operation_time)
                        print(f"Finished operating on {patient.name}.")
                    else:
                        # No more patients left to process
                        break

                # Reset the event after processing all patients
